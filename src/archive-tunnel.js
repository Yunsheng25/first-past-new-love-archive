...
export function mountArchiveTunnel(root, data, options = {}) {
  ...
  const documentRef = options.documentRef ?? root.ownerDocument ?? (typeof document === "undefined" ? null : document);
  ...
  const cards = occurrences.map((occurrence, index) => {
    ...
    card.type = "button";
    card.className = "archive-tunnel-card";
    ...
    layer.append(card);
    return { card, image, occurrence, click: null };
  });
  ...
  function resize() { if (!registry.destroyed) render(); }
